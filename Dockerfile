...
# cd xgboost
# git checkout amd-sep
# git submodule update --init --recursive
# docker build --build-arg GITHUB_TOKEN=${GITHUB_TOKEN} -t mun-node-0.acp.amd.com:8001/xgboost:amd-sep -f Dockerfile .
...
#RUN git clone https://${GITHUB_TOKEN}@github.com/AMD-AI/xgboost.git -b amd-sep --recurse-submodules \
# && cd xgboost \
RUN rm -fr build \
 && mkdir build \
 && cd build \
